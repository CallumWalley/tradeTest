...
public class UITradeReceiver : Control
{
	...
	static readonly PackedScene tradeRoute = (PackedScene)GD.Load<PackedScene>("res://templates/GUI/Components/UITradeRoute.tscn");

	public override void _Ready()
	{
		// Assign freighter counting icons.
		freightersTotal = GetNode<UIResource>("Trade/FreighterPool/Pool");
		freightersRequired = GetNode<UIResource>("Trade/FreighterPool/Used");
		freightersTotal.Init(tradeReceiver.freightersTotal);
		freightersRequired.Init(tradeReceiver.freightersRequired);

		list = GetNode<VBoxContainer>("Trade/Scroll/VBox");        
	}
